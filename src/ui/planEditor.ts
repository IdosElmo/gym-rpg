...
function presetList(store: DataStore): string {
  ...
  return `<ul class="pl-lib-list">${items}</ul>
    ...
    <p class="gc-note dim">בחירה בתוכנית מוכנה מחליפה את הטיוטה הנוכחית. שום דבר לא נשמר עד לחיצה על 💾 שמירה, וההיסטוריה נשמרת בכל מקרה. שמירת תוכנית ⭐ מקפיאה את התוכנית שבעריכה כמו שהיא — היא נשמרת בחשבון שלכם ולא במכשיר: אפשר לחזור אליה מכאן בכל רגע, ומכל מכשיר שמחובר לאותו חשבון. רק אתם רואים אותה — התוכניות המוכנות של האפליקציה לא משתנות.</p>`;
}
